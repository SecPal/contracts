...
const preCommitConfig = readFileSync(
  `${repositoryRoot}/.pre-commit-config.yaml`,
  'utf8'
)
const preflightScript = readFileSync(
  `${repositoryRoot}/scripts/preflight.sh`,
  'utf8'
)
const setupScript = readFileSync(
  `${repositoryRoot}/scripts/setup-pre-commit.sh`,
  'utf8'
)
...
const expectedVersion = declaredVersion

const lockfileDeclaredVersion =
  packageLock.packages?.['']?.devDependencies?.['markdownlint-cli'] ?? ''
if (lockfileDeclaredVersion !== expectedVersion) {
  fail(
    `package-lock.json root package must pin markdownlint-cli to ${expectedVersion}, found ${lockfileDeclaredVersion || 'missing'}.`
  )
}

const lockedPackageVersion =
  packageLock.packages?.['node_modules/markdownlint-cli']?.version ?? ''
if (lockedPackageVersion !== expectedVersion) {
  fail(
    `package-lock.json must resolve node_modules/markdownlint-cli to ${expectedVersion}, found ${lockedPackageVersion || 'missing'}.`
  )
}
...
if (
  !hook.includes('entry: node node_modules/markdownlint-cli/markdownlint.js')
) {
  fail(
