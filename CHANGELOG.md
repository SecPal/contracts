...
- Expanded `README.md` coverage note to enumerate all undocumented admin-heavy routes: listed all five RBAC role-management sub-paths (`GET`, `POST`, `GET /{id}`, `PATCH /{id}`, `DELETE /{id}` under `/v1/roles`), all four user-role assignment sub-paths (`GET`, `POST`, `DELETE /{role}`, `PATCH /{role}/extend` under `/v1/users/{user}/roles`), and all four direct user-permission sub-paths (unchanged from the previous entry); compare with `routes/api.php` in `SecPal/api` when auditing coverage.
