...
- Documented the current onboarding runtime surface in `docs/openapi.yaml` (closes #180), including the public bootstrap endpoints (`GET /onboarding/validate-token`, `POST /onboarding/complete`), the authenticated employee dossier endpoints (`GET /onboarding/steps`, `GET /onboarding/forms/{templateId}`, `GET /onboarding/submissions`, `POST /onboarding/submissions`, `PATCH /onboarding/submissions/{submissionId}`, `POST /onboarding/submissions/{submissionId}/files`), the HR review and onboarding confirmation actions (`POST /admin/onboarding/submissions/{submissionId}/approve`, `POST /admin/onboarding/submissions/{submissionId}/reject`), and the explicit `onboarding_workflow.status` field on employee resources so clients can distinguish dossier completeness from activation readiness; relates to route-drift follow-up SecPal/frontend#731
