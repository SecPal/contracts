...
- Documented the onboarding upload `document_subtype` requirement for `id_document` attachments in `docs/openapi.yaml`, including explicit subtype enums, request examples, and `422` `ValidationProblem` examples covering missing `document_subtype`, non-editable submissions, and upload failures surfaced before validation
