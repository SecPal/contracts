...
- **Breaking:** Replaced the Android-only authenticated push runtime contract in [docs/openapi.yaml](/home/secpal/code/SecPal/contracts/docs/openapi.yaml) with a channel-aware notification-installation model (closes #288; parent epic `SecPal/api#1131`): renamed the canonical authenticated surface from `PUT`/`DELETE /me/push-devices/{installationId}` to `PUT`/`DELETE /me/notification-installations/{installationId}`; removed shared `platform`/`provider` naming in favor of the canonical `channel` discriminator (`android_fcm` and `web_push`); replaced bootstrap `features.android_push` and top-level `android_push` metadata with `features.notification_channels` and per-channel `notification_channels.android_fcm` / `notification_channels.web_push` public runtime metadata; added explicit Web Push bootstrap, registration, revocation, validation, and conflict examples alongside Android FCM; renamed the fail-closed `409` variants to `NOTIFICATION_CHANNEL_UNSUPPORTED` and `NOTIFICATION_RUNTIME_STATE_INVALID`; and bumped the bootstrap `schema_version` example from `2` to `3` so clients can reject stale channel metadata deterministically. Obsolete Android-only aliases are intentionally not preserved under the current `0.x` contract policy.
