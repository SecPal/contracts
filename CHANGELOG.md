...
- Documented the authenticated German address reference-data contract in `docs/openapi.yaml`, including `GET /addresses/de/streets`, `GET /addresses/de/localities`, and `GET /addresses/de/status`, the real message-only Laravel `401`/`403`/`429`/`500` response shapes for those endpoints, and the verified-endpoint guard update in `scripts/check-openapi-verified-endpoints.mjs`
