...
- Corrected the contracts repo's workflow-governance baseline so reusable workflow caller jobs follow the valid timeout policy, and pinned shared `.github` reusable workflows in `.github/workflows/quality.yml` to an immutable commit SHA with matching governance refs for reproducible AI-instructions and markdown-lint checks.
