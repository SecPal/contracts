...
- Documented the canonical customer-owned Android push contract in `docs/openapi.yaml` (closes #286; parent epic `SecPal/api#1121`): extended `GET /bootstrap` with optional `android_push` public runtime metadata limited to safe client fields (`api_key`, `project_id`, `application_id`, `sender_id`) plus metadata revisioning; added authenticated `PUT`/`DELETE /me/push-devices/{installationId}` registration, token-rotation, and revocation endpoints for the generic Android app; documented `422` invalid-payload and `409` unsupported/stale-runtime failure bodies with concrete examples; and stated that obsolete Android push/mobile fields are not preserved as compatibility aliases under the current `0.x` policy.
