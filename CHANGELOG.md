...
- Fixed `PasskeyAuthenticatorSelection.resident_key` description: now describes the enrollment `resident_key` option correctly (controls discoverable-credential creation during registration) instead of incorrectly stating sign-in ceremony behavior; updated `PasskeyAuthenticationPublicKeyOptions` description to note that the discoverable-only contract tracks `SecPal/api#1101` and that clients should regenerate only after that change is deployed.
